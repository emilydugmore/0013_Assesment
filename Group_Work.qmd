...
*   [*Emily Dugmore*  (ID: 25060351)](https://github.com/EmilyDeeb/0013_Assesment)  
*   [*Julissa Tabata Paredes Condor*  (ID: 25153376)](https://github.com/EmilyDeeb/0013_Assesment)  
*   [*Jiaying LI*  (ID: 25211516)](https://github.com/EmilyDeeb/0013_Assesment)  
*   [*Emily Deeb*  (ID: 25127081)](https://github.com/EmilyDeeb/0013_Assesment)  
...
Eg:  
- Method: "We were unsure if our method for imputing missing revenue data was sound. Any feedback on alternative approaches would be appreciated."  
- Visualization: "We struggled to choose a compelling color scheme for the choropleth map (Figure x.x). Does it effectively communicate the spatial pattern?"  
- Narrative: "Is the logical flow of our 'Policy Recommendation' section clear? Does the evidence strongly support our final conclusion?"  
...
*   **Pearson's Correlation ($r$)**: `{python} f"{corr_coef:.3f}"`
*   **P-value ($p$)**: `{python} p_value_text`

**Interpretation**: The correlation coefficient is very close to zero, suggesting almost no linear relationship. However, the result is statistically significant at the 5% level, indicating the weak negative correlation is unlikely due to random chance.
:::
...
- Average Airbnb revenue is much higher than council tax (≈ £28k vs ≈ £1.9k). The map shows a clear mismatch: the highest-earning boroughs (Westminster, Kensington & Chelsea, City of London) pay some of the lowest council taxes, while outer boroughs with weak Airbnb income face higher council tax and very few listings. 
- In short, Airbnb profitability is concentrated in the centre, and council tax has almost no bite where the profits are highest.  
...
::: {.callout-note icon="false"}  
This regression is based on a sample of **`{python} f"{num_used:,}"`** listings (out of a total of `{python} f"{num_total:,}"` professional listings). The approximate linear relationship is given by:  
...
```{python}
#| tbl-cap: "Average Profit Loss per Borough"
#| echo: false
#| output: asis
# Latex format
...
# Create a clean column name to plo
...
London is adding about 35k homes per year, while Airbnb adds 5–7.2k new listings a year – roughly 14–20% of annual housing output. A noticeable share of homes is being diverted to short-term lets rather than long-term housing. (@greaterlondonauthority)  

Based on our findings, short-term letting is much more profitable than long-term renting, giving owners a clear financial incentive to convert units to Airbnb. (Finding From Q1)  
...
However, the city should also focus on preventing the future expansion of Airbnb units. For that, and based on the Los Angeles case studied by Dayne Lee (@DayneLee), the proposal should be complemented by:    
